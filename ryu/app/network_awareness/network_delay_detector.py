...
class NetworkDelayDetector(app_manager.RyuApp):
    ...
    def _send_echo_request(self):
        ...
        for datapath in list(self.datapaths.values()):
            ...
            # convert time to bytes with struct
            t = time.time()
            bytes = struct.pack('d', t)
    ...
    def echo_reply_handler(self, ev):
        ...
        try:
            latency = now_timestamp - struct.unpack('d', ev.msg.data)[0]
    ...
    def get_delay(self, src, dst):
        ...
        try:
            ...
            delay = fwd_delay - src_latency/2 - dst_latency/2 - 0.003
            return max(delay, 0)
        except:
            return float('inf')
        ...
        if setting.TOSHOW and self.awareness is not None:
            ...
            for src in self.awareness.graph:
                for dst in self.awareness.graph[src]:
                    ...
                    self.logger.info("%s<-->%s : %.12f" % (src, dst, delay))
